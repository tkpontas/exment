...
class ExpansionGrid extends DefaultGrid
{
    ...
    protected function setGridColumn($grid_column, $custom_view_column)
    {
        ...
        $grid_column->expand(function ($model) use ($relation, $child_view) {
            ...
            foreach ($children_values as $children_value) {
                ...
                $child_values[] = $child_view
                    ->custom_view_columns
                    ->map(function ($child_view_column) use (
                        $child_view,
                        $children_value
                    ) {
                        ...
                        $item = $item->options([
                            ...
                            'view_pivot_column' =>
                                $child_view_column
                                    ->view_pivot_column_id ?? null,
                            'view_pivot_table' =>
                                $child_view_column
                                    ->view_pivot_table_id ?? null,
                            'header_align' =>
            ...
            /** @var Collection<int, string|null> $child_labels */
            $child_labels = $child_view
                ->custom_view_columns
                ->map(function ($child_view_column) {
    ...
    public static function setViewForm(
        ...
        $form->hasManyTable(
            'custom_view_columns',
            exmtrans("custom_view.custom_view_columns"),
            function ($form) use ($custom_table): void {
                ...
                $relations =
                    CustomRelation::getRelationsByParent(
                        $custom_table,
                        RelationType::ONE_TO_MANY
                    );

                $child_tables = $relations
                    ->mapWithKeys(function ($relation): array {
                ...
                $form->select(
                    'child_table_id',
                    exmtrans(
                        "custom_view.child_table_id"
                    )
                )
                ->help(
                    exmtrans(
                        "custom_view.help.child_table_id"
                    )
                )
        ...
        ->required()
        ->setTableColumnWidth(5, 3, 2, 2)
        ->rowUpDown('order', 10)
        ->descriptionHtml(
            exmtrans(
                "custom_view.description_custom_view_columns"
            )
        );

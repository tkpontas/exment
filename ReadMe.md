...
次期バージョンで取り込まれるかもしれない不具合修正の先行体験
- hotfixfeature/issue_1662 (「見出し表示方法」に「縦並び」を選択すると 編集画面で子テーブルの入力欄に見出しが表示される不具合の解消)
- hotfixfeature/issue_1663 (フォーム設定のフィールド種類が「表示専用」の場合に、値が「初期値」に置き換わる不具合の解消)
